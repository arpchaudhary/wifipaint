	...
	outStr = ""
        for r in range(self.rows):
            ...
            for c in self.grid[r]:
                ...
                val = '';
		if c == self.empty:
                    val = '%s' % string.lowercase[random.randint(0,len(string.lowercase)-1)]
		else:
                    val = '%s' % c
		outStr += val;
		res[point] = val
...
#custom_list=['arpit',''],['abhay',''],['varun','']
print custom_list
#sys.exit()
a = Crossword(7, 7, '-', 5000, custom_list)
a.compute_crossword(2)
#print a.word_bank()
#print a.solution()
grid = a.word_find();

print json.dumps(a.word_find())
#print a.display()
#print a.legend()
